<script lang="ts">
  ...
  const cButtonCenter = "";
  const cScriptCenter =
    "w-full my-auto text-center truncate overflow-hidden text-ellipsis before:content-['▶']";
  ...
  function handleClick() {
    switch (targetType) {
      ...
      case GOTYPE.SITEDATES:
        goto(`/api/sitedate/${targetId}/${targetIdSecondary}`);
        break;
      case GOTYPE.SITES:
...
<div class={classesControlBody}>
  <button
    ...
    onclick={handleClick}
    title={`Proceed to ${GOTYPE[targetType]}`}
    disabled={controlDisabled || targetType === GOTYPE.UNKNOWN}
