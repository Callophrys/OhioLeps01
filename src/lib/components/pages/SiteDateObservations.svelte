<script lang="ts">
  ...
  const handleChange = (
    ...
    editingTotal = section
      ? Array.from(section.querySelectorAll("[type=text]")).reduce(
          (t: number, o: any) => t + (isNaN(o.value) ? 0 : Number(o.value)),
          0,
        )
      : 0;
  ...
  function clickSubmitReview(e: any) {
    if (e.currentTarget?.form) {
      ...
      if (
        (targetForm.elements.namedItem("confirm") as HTMLInputElement).value ===
        "true"
      ) {
  ...
  function onSubmitDelete(
    ...
    if (
      (targetForm.elements.namedItem("deleteOn") as HTMLInputElement).value ===
      "true"
    ) {
  ...
  function modalComponentAdd(): void {
    ...
    const modal: ModalSettings = {
      ...
      value: {
        ...
        siteDateId: data.siteDateObservation.siteDateId,
  ...
  // TODO: is this used?
  function modalComponentSiteDateObservation(isNewRecord: boolean): void {
    let isSiteDate = true;
  ...
  function modalComponentEdit(
    ...
    const c: ModalComponent = { ref: ModalSiteDateObservation };
    const sdoId =
      Number(e.currentTarget.dataset.sitedateobservationid) ??
      data.siteDateObservation.id;
    const sdo =
...
{#snippet controlsOutside()}
  <YearWeek
    year={new Date(siteDate.recordDate).getFullYear()}
  ...
  >
    <div class="flex flex-row">
      ...
      <!--
                ...
                -->
      <!-- TODO: Maybe move this to site date pane -->
      <SitePicker
        {currentSiteId}
        currentCountyId={-1}
        currentSiteDateId={-1}
...
{#snippet reviewSpecimenViewMultiple(sdo: SiteDateObservationChecklist)}
  ...
  {:else if isReviewable(sdo, $page.data.user)}
    ...
    >
      {#if !sdo.confirmById}
        <button type="submit" title="Needs review" onclick={clickSubmitReview}
          >🌎</button
        >
        <input hidden name="confirm" value={true} />
      {:else if sdo.confirmed}
        <button type="submit" title="Reviewed" onclick={clickSubmitReview}
          >🔐</button
        >
      ...
      <div class={specimenClassesMultiple(chkSdo)}>
        <div class="pl-1 flex flex-row justify-between">
          <div class="flex flex-row justify-start">
            <div class="w-6">
              {chkSdo.deleted ? "❌" : chkSdo.confirmed ? "🔐" : "🔓"}
            </div>
            ...
            {#if chkSdo.deleted}
              <div class="w-44 pr-2 pb-0.5 text-right">
                ID Method: {@html chkSdo.idCode ?? "&varnothing;"}
              </div>
        ...
        <div class="pl-8 flex flex-wrap">
          ...
          <div class={cSectionRowClasses}>
            ...
            <div id={`sum_${chkSdo.id}`} class={cSectionSpanDivClasses}>
              {chkSdo.total}
            </div>
          </div>
        </div>
    ...
    <div class={`flex flex-row ${specimenClassesMultiple(chkSdo)}`}>
      <div class="basis-6">
        ...
        {:else}
          {#if chkSdo.deleted}
            <div class="w-6">
              <button disabled class="cursor-not-allowed" aria-labelledby=" "
                >&nbsp;</button
              >
            </div>
          {:else}
            <div class="w-6 text-center content-[2714]">
              ...
              {:else}
                <button
                  ...
                  disabled={isEditing}
                  data-siteDateObservationId={chkSdo.id}>✎</button
                >
              {/if}
            </div>
          {/if}
...
<Container {body} bodyClasses="overflow-hidden" outerBodyClasses="pl-0" />
