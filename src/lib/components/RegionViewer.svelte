<script lang="ts">
  ...
  function initialize(psvgs: any, svgId: string) {
  ...
  function getViewBox(psvgs: any): string {
    let xmin = Infinity;
    let xmax = -Infinity;
    let ymin = Infinity;
    let ymax = -Infinity;
  ...
  function addListeners(node: HTMLDivElement) {
...
<Splitpanes style="height: 100%">
  <Pane minSize={5}>
    <Container head={leftHead} body={leftBody} tail={leftTail} />
  </Pane>
  <Pane size={50} minSize={3}>
    <Container head={rightHead} body={rightBody} tail={rightTail} />
  </Pane>
</Splitpanes>
