<script lang="ts">
  // import ModalSections from "$lib/components/ModalSections.svelte";
          ...
          fetch(componentUrl, {
            ...
            .then((data) => {
              if (data.status === 200) {
                ...
                let retSiteId =
                  Array.isArray(tmpSiteId) && tmpSiteId.length > 1
                    ? tmpSiteId[1]
                    : tmpSiteId;
  ...
  // function modalComponentTransect(site: Site | null): void {
  //   const c: ModalComponent = {
  //     ref: ModalSite,
  //   };
  //   const componentTitle = "Transect Configuration";
  //   const componentUrl = "../transect";
  //   // const componentUrlAdd = "../transect"; // POST
  //   // const componentUrlUpdate = "../transect/${transectId}"; // PUT
  //   // const componentUrlRemove = "../transect/${transectId}"; // DELETE
  //
  //   const componentValues = isNewRecord
  //     ? {
  //         site: null,
  ...
  function modalComponentSiteDate(
    ...
    const componentTitle = isNewRecord
      ? "Add New Date Record"
      : `Edit Date Record - ${formatDate(siteDate?.recordDate ? new Date(siteDate.recordDate).toISOString() : new Date().toISOString())}, ${siteDate?.recordDate}`;
    const componentUrl = isNewRecord
      ? "../sitedate/-1?/createSiteDate"
      : `../sitedate/${siteDate?.id}?/updateSiteDate`;
  ...
  let currentSiteDate: SiteDateYear | null = $state(
    data.site.siteDates.length ? data.site.siteDates[0] : null,
  );
  ...
  //   data.site.sections = [
  //     ...data.site.sections,
  //     { sectionIndex: 0, labelShort: "", labelLong: "", description: "" },
  //   ];
  // }
  //
  // function removeChild(index) {
  //   data.site.sections = data.site.sections.filter((_, i) => i !== index);
  // }
  //
  // // Provide context to the form
  // setContext("parentData", { addChild, removeChild });

  // // Modal settings
  // function openModal() {
  //   modalStore.trigger({
  //     type: "component",
  //     component: { ref: ModalSections },
  //     title: "Transect Configuration",
  //     body: "Add, edit, or remove transects using the form below",
  //     value: data.site,
  //     response: null,
  //     // response: (r) => {
  //     //   if (typeof r === "object") {
...
<CountySite
  bind:county={data.site.county.name}
  bind:site={data.site.siteName}
  bind:filterByCounty
/>
  ...
  <div class="flex flex-row justify-between gap-1 md:gap-2">
    <div class="flex flex-row">
      <GoBack
        ...
        bind:targetType={GOTYPE.COUNTYSITES}
      ...
      />
      <GoNext
        targetId={currentSiteId}
        targetType={GOTYPE.SITEDATES}
        targetIdSecondary={currentSiteDateId}
        controlBody="scale-90"
        controlDisabled={data.site.siteDates.length < 1}
        buttonCenter=""
        scriptCenter=""
        labelledby=""
      />
      <CountyPicker
        bind:currentCountyId
        bind:currentSiteId
        bind:filterByCounty
        controlBody="scale-90 -translate-x-2"
        heading={null}
        dropdownPointers={null}
        controlOuter={null}
        buttonLeft={null}
        buttonCenter={null}
        buttonRight={null}
      ...
      <SitePicker
        ...
        heading={null}
        controlOuter=""
        controlBody="scale-90 -translate-x-8"
        buttonLeft=""
        buttonCenter=""
        buttonRight=""
        scriptCenter=""
    ...
    <div class="content space-y-2">
      <div>
        ...
        <div class="pl-4">
          <div>
            County:
            {currentSite?.county.name ?? ""}
          </div>
          <div>
            Township:
            {currentSite?.township ?? ""}
          </div>
          <div>
            Location Zip:
            {currentSite?.locationZip ?? ""}
          </div>
        </div>
      </div>
      ...
      <div>
        ...
        <div class="pl-4">
          ...
          {#if currentSite?.siteAddress2}
            <div>
              Street Address2:
              {currentSite.siteAddress2}
            </div>
          {/if}
          <div>
            City:
            {currentSite?.siteCity ?? ""}
          </div>
          <div>
            State:
      ...
      <div>
        ...
        {#if $page.data?.user && ($page.data.user.role === ROLE.SUPER || $page.data.user.role === ROLE.ADMIN)}
          <div class="pl-4">
            <div>
              Address:
              {currentSite?.personAddress ?? ""}
            </div>
            {#if currentSite?.personAddress2}
              <div>
                Address2:
                {currentSite?.personAddress2 ?? ""}
              </div>
            {/if}
            <div>
              City:
              {currentSite?.personCity ?? ""}
            </div>
            <div>
              State:
              {currentSite?.personState ?? ""}
            </div>
            <div>
              Zip:
              {currentSite?.personZip ?? ""}
            </div>
            <div>
              Phone:
              {currentSite?.personPhone ?? ""}
            </div>
            <div>
              Email:
              {currentSite?.personEmail ?? ""}
            </div>
          </div>
        {/if}
      </div>
      ...
      <div>
        ...
        <div class="pl-4">
          <div>
            latitudeStart:
            {currentSite?.latitudeStart ?? ""}
          </div>
          <div>
            latitudeEnd:
            {currentSite?.latitudeEnd ?? ""}
          </div>
          <div>
            longitudeStart:
            {currentSite?.longitudeStart ?? ""}
          </div>
          <div>
            longitudeEnd:
            {currentSite?.longitudeEnd ?? ""}
          </div>
        </div>
      </div>
      ...
      <div>
        <div>
          Alternate Person:
          {currentSite?.altPerson ?? ""}
        </div>
        {#if currentSite?.altPerson && $page.data?.user && ($page.data.user.role === ROLE.SUPER || $page.data.user.role === ROLE.ADMIN)}
          <div class="pl-4">
            <div>
              Address:
              {currentSite?.altPersonAddress ?? ""}
            </div>
            {#if currentSite?.altPersonAddress2}
              <div>
                Address2:
                {currentSite?.altPersonAddress2 ?? ""}
              </div>
            {/if}
            <div>
              City:
              {currentSite?.altPersonCity ?? ""}
            </div>
            <div>
              State:
              {currentSite?.altPersonState ?? ""}
            </div>
            <div>
              Zip:
              {currentSite?.altPersonZip ?? ""}
            </div>
            <div>
              Phone:
              {currentSite?.altPersonPhone ?? ""}
            </div>
            <div>
              Email:
              {currentSite?.altPersonEmail ?? ""}
            </div>
          </div>
        {/if}
      </div>
      ...
      <div>
        ...
        <ul class="pl-4">
          {#each data.site.siteStatuses as siteStatus}
            <li>
              {siteStatus.year}:
              {siteStatus.statusCode.description ?? ""}
            </li>
          {/each}
        </ul>
      </div>
      ...
      <!-- <div> -->
      <!--     <div> -->
      <!--         <div>Configure Transects</div> -->
      <!--         <button class="button {parent.buttonPositive}">Add</button> -->
      <!--     </div> -->
      <!--     <div class="pl-8"> -->
      <!--         <div class="flex flex-row"> -->
      <!--             <button class="btn {parent.buttonNegative}">X</button> -->
      <!--             <div class="flex flex-row"> -->
      <!--                 <div class="w-4">0</div> -->
      <!--                 <div>Transect Description</div> -->
      <!--             </div> -->
      <!--         </div> -->
      <!--     </div> -->
      <!-- </div> -->
          ...
          <div class="pl-4 flex flex-row space-x-8">
            <div>
              <div>
                ...
                {data.site?.createdAt
                  ? formatDate(
                      new Date(data.site.createdAt).toISOString(),
                      "medium",
                      "short",
                    )
                  : ""}
            ...
            <div>
              <div>
                ...
                {data.site?.updatedAt
                  ? formatDate(
                      new Date(data.site.updatedAt).toISOString(),
                      "medium",
                      "short",
                    )
                  : ""}
      ...
      {#if $page.data?.user}
        {#if $page.data.user.role === "SUPER" || $page.data.user.role === "ADMIN"}
          ...
          <button
            ...
            ><span class="text-success-400">✚</span>&nbsp;Edit Site</button
          >
          <!-- <button -->
          <!--   type="button" -->
          <!--   class="btn h-10 variant-soft" -->
          <!--   onclick={() => openModal()} -->
          <!--   title="Edit Transects" -->
          <!--   ><span class="text-success-400">✚</span>&nbsp;Edit Transects</button -->
          <!-- > -->
        {/if}
        {#if $page.data.user.role === "SUPER" || $page.data.user.role === "ADMIN" || $page.data.user.role === "ENTRY"}
          <button
            ...
            title="Add a new date record with observations"
            ><span class="text-success-400">✚</span>&nbsp;Add Observations</button
          >
        {/if}
      {/if}
      <button
        ...
        onclick={exportToCSV}
        title="Downloadable"
        ><span class="text-success-400">✚</span>&nbsp;Export to CSV</button
      >
    </div>
  </div>
{/snippet}
